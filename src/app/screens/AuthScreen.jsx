...
import { mapProfileToUser, upsertProfile, uploadProfileImage } from "../data/profileStore";
...
export default function AuthScreen() {
  ...
  const syncProfileRow = async ({ id, email: userEmail, fullName: nextFullName, avatarUrl }) => {
    try {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (activeTab === "signup") {
        ...
        if (!data.session) {
          toast.success("Account created. Check your UTA email to confirm your account, then add your profile photo from your profile page.");
          setIsLoading(false);
          return;
        }

        const avatarUrl = await uploadProfileImage(profilePictureFile, data.user.id);
        const profileRow = await syncProfileRow({
          id: data.user.id,
        ...
        login(mapProfileToUser(data.user, profileRow, avatarUrl));
      ...
      login(mapProfileToUser(data.user, null, profilePicture));
  ...
  const isFormValid =
    emailIsValid &&
    password &&
    (activeTab === "login" || (activeTab === "signup" && fullName.trim() && profilePictureFile));
